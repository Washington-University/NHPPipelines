...
FreeSurferSubcorticalLabels=${HCPPIPEDIR_Config}/FreeSurferSubcorticalLabelTableLut.txt
...
${FSLDIR}/bin/flirt -interp nearestneighbour -in "${MNIFolder}"/"${ribbon}" -ref $standard -applyisoxfm ${StandardResolution} -out "${ROIsFolder}"/ribbon."${StandardResolution}"
...
imcp "${MNIFolder}"/"${wmparc}" "$ROIsFolder"/"$wmparc"."${StandardResolution}".nii.gz
...
# Create Atlas Grayordinate dense scalar (added by Takuya Hayashi June 2017)
${Caret7_Command} -cifti-create-dense-scalar "$ResultsFolder"/Atlas_Greyordinates.dscalar.nii -volume "$ROIsFolder"/Atlas_ROIs."$StandardResolution".nii.gz "$ROIsFolder"/Atlas_ROIs."$StandardResolution".nii.gz -left-metric ${DownSampleFolder}/${Subject}.L.atlasroi.${LowResMesh}k_fs_LR.shape.gii -roi-left ${DownSampleFolder}/${Subject}.L.atlasroi.${LowResMesh}k_fs_LR.shape.gii -right-metric ${DownSampleFolder}/${Subject}.R.atlasroi.${LowResMesh}k_fs_LR.shape.gii -roi-right ${DownSampleFolder}/${Subject}.R.atlasroi.${LowResMesh}k_fs_LR.shape.gii
