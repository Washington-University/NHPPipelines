...
DEFAULT_STUDY_FOLDER="/mnt/FAI1/MRI/M.Mulatta/HCP-RIKEN"
DEFAULT_SUBJECT_LIST=""
DEFAULT_ENVIRONMENT_SCRIPT="/mnt/FAI1/devel/NHPHCPPipeline/Examples/Scripts/SetUpHCPPipelineNHP.sh"
DEFAULT_RUN_LOCAL="NONE"
DEFAULT_FIX_DIR="/usr/local/fix1.06"
...
main() {
	...
	do
		echo "Subejct: ${Subject}"
		# Added by Takuya Hayashi 2016/08/22 from here:
		. $StudyFolder/$Subject/RawData/hcppipe_conf.txt
		ScanList="$Tasklist"
		if [ "x$ScanList" = "x" ] ; then
			for i in $StudyFolder/$Subject/MNINonLinear/Results/rfmri*; do ScanList="$ScanList `basename $i`"; done
		else
			ScanList="`echo $ScanList | sed -e 's/@/ /g'`"
		fi
		...
		#do
		for rfmri in $ScanList		
		do
			#echo "  ${Condition}"
			#rfmri=`basename $rfmri`
			rfmri=`remove_ext $rfmri`
			echo " $rfmri"; 
			...
			#do
				...
				InputDir="${StudyFolder}/${Subject}/MNINonLinear/Results/$rfmri"
				InputFile="${InputDir}/${rfmri}"

				bandpass=2000
				
				if [ "${RunLocal}" == "TRUE" ]
				then
					echo "About to run ${FixDir}/nhphcp_fix ${InputFile} ${bandpass}"
					queuing_command=""
				else
					echo "About to use fsl_sub to queue or run ${FixDir}/nhphcp_fix ${InputFile} ${bandpass}"
					queuing_command="${FSLDIR}/bin/fsl_sub ${QUEUE} -l ${StudyFolder}/${Subject}/logs"
				fi

				${queuing_command} ${FixDir}/nhphcp_fix ${InputFile} ${bandpass}
			#done
