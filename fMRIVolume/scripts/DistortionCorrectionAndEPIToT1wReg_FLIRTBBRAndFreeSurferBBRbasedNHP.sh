...
BBR=1 # 0: does not use BBR because it is not robustly effective in NHP, instead use flirt with T2 and EPI. It also uses epi_reg_dof_nobber Takuya Hayashi Dec 2017
...
case $DistortionCorrection in
    ...
    ${FIELDMAP_METHOD_OPT} | ${SIEMENS_METHOD_OPT} | ${GENERAL_ELECTRIC_METHOD_OPT})
        ...
        if [ $DistortionCorrection = "${FIELDMAP_METHOD_OPT}" ] || [ $DistortionCorrection = "${SIEMENS_METHOD_OPT}" ] ; then
            ...
            ${GlobalScripts}/SiemensFieldMapPreprocessingAllNHP.sh \
        ...
        if [[ X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) > 1.25 ) {1}" | bc -l` = X1 || X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) < 0.75 ) {1}" | bc -l` = X1 ]] ; then
            ...
            if [ "$BBR" = "1" ] ; then
              ...
              ${HCPPIPEDIR_Global}/epi_reg_dof_nobbr --dof=${dof} --epi=${WD}/Scout_brain.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} --out=${WD}/${ScoutInputFile}_undistorted --fmap=${WD}/FieldMap.nii.gz --fmapmag=${WD}/Magnitude.nii.gz --fmapmagbrain=${WD}/Magnitude_brain.nii.gz --echospacing=${DwellTime} --pedir=${UnwarpDir}
            fi
        else
            #Takuya Hayashi inserted for NHP
            if [ "$BBR" = "1" ] ; then
              # register scout to T1w image using fieldmap
              ...
              # NHP EPI data does not have clear contrat between gray/white thus bbr is not effective - Takuya Hayashi inserted for NHP
              log_Msg "Run epi_reg_dof_nobbr"
              ${HCPPIPEDIR_Global}/epi_reg_dof_nobbr --dof=${dof} --epi=${WD}/Scout_brain.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} --out=${WD}/${ScoutInputFile}_undistorted --fmap=${WD}/FieldMap.nii.gz --fmapmag=${WD}/Magnitude.nii.gz --fmapmagbrain=${WD}/Magnitude_brain.nii.gz --echospacing=${DwellTime} --pedir=${UnwarpDir}
            fi
        fi
    ...
    ${SPIN_ECHO_METHOD_OPT})
        ...
        if [ -e ${FreeSurferSubjectFolder}/${FreeSurferSubjectID}_1mm ] ; then
           # Takuya Hayashi deleted Oct 3rd 2017
            #cp ${ScoutInputName}.nii.gz ${WD}/Scout.nii.gz
            #${FSLDIR}/bin/flirt -interp spline -dof 6 -in ${WD}/Scout.nii.gz -ref ${T1wImage} -omat "$WD"/Scout2T1w.mat -out ${WD}/Scout2T1w.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30
            #${FSLDIR}/bin/convert_xfm -omat "$WD"/T1w2Scout.mat -inverse "$WD"/Scout2T1w.mat
            ...
            #Takuya Hayashi inserted Oct 3rd 2017  T2w-based registration and brain masking
            ${FSLDIR}/bin/applywarp --rel --interp=spline -i ${ScoutInputName}.nii.gz -r ${ScoutInputName}.nii.gz  -w ${WD}/WarpField.nii.gz -o ${WD}/${ScoutInputFile}_undistorted_init
            ${FSLDIR}/bin/flirt -interp spline -dof 6 -in ${WD}/${ScoutInputFile}_undistorted_init -ref ${WD}/../../T2w/T2w -omat "$WD"/Scout2T2wOrig.mat -out ${WD}/Scout2T2wOrig.nii.gz -nosearch
            ${FSLDIR}/bin/convert_xfm -omat "$WD"/Scout2T2w.mat -concat "$WD"/../../T2w/xfms/acpc.mat "$WD"/Scout2T2wOrig.mat
            ${FSLDIR}/bin/convert_xfm -omat "$WD"/T2w2Scout.mat -inverse "$WD"/Scout2T2w.mat
            ${FSLDIR}/bin/applywarp --interp=nn -i ${WD}/${T1wBrainImageFile} -r ${ScoutInputName} --premat="$WD"/T2w2Scout.mat -o ${WD}/Scout_brain_mask.nii.gz    
            fslmaths ${WD}/Scout_brain_mask.nii.gz -bin ${WD}/Scout_brain_mask.nii.gz
            fslmaths ${WD}/${ScoutInputFile}_undistorted_init -mas ${WD}/Scout_brain_mask.nii.gz ${WD}/${ScoutInputFile}_undistorted
            #Takuya Hayashi inserted Oct 3rd 2017 until here
        else
            # create a spline interpolated image of scout (distortion corrected in same space)
            log_Msg "create a spline interpolated image of scout (distortion corrected in same space)"
        ...
        # Takuya Hayashi inserted Oct 3rd 2017 from here
        if [ $BBR = 0 ] ; then
         flirt -interp spline -dof 6 -in ${WD}/${ScoutInputFile}_undistorted -ref ${WD}/${T1wBrainImageFile} -omat ${WD}/${ScoutInputFile}_undistorted2T1w_init.mat -out ${WD}/${ScoutInputFile}_undistorted2T1w_init -init "$WD"/Scout2T2w.mat
         applywarp --interp=spline -i ${WD}/${ScoutInputFile}_undistorted -r ${T1wImage} --premat=${WD}/${ScoutInputFile}_undistorted2T1w_init.mat -o ${WD}/${ScoutInputFile}_undistorted2T1w_init
...
if [ -e ${FreeSurferSubjectFolder}/${FreeSurferSubjectID}_1mm ] ; then
 ...
 if [ "$BBR" = "1" ] ; then  # Takuya Hayashi inserted, Dec 2017 
  log_Msg "Use refined registration with FS BBR"
 ...
 else # if FREESURFERBBR is not used. Instead use T2 and EPI registraion. Takuya Hayashi Dec 2017
   log_Msg "Use refined registration with EPI and T2w"
   T2wRestoreImage="`dirname $T1wRestoreImage`/T2w_acpc_dc_restore.nii.gz"
   flirt -in $ScoutImageFile -ref $T2wRestoreImage -dof 6 -nosearch -omat ${WD}/fMRI2str_refinement.mat
 fi
else
